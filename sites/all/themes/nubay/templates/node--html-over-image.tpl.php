...
<?php if ($page == 0): ?>
  ...
  <?php
    ...
    if(!empty($node->field_upload_background_image)&& in_array('2',$background_options)){
	  if(!empty($node->field_image_width)){
        $canvas_back_img_css .= 'background-position: 50% 50%;background-size: cover; background-image:url('.file_create_url($node->field_upload_background_image['und'][0]['uri']).'); background-repeat:no-repeat;';
	  }else{
	...
	if(empty($node->field_text_box_location_on_image)){ 
	  ...
	  if(!empty($node->field_image_width)){
	    $canvas_css .= 'display: table; width:'.$node->field_image_width['und'][0]['value'].'px;';
	  }else{
